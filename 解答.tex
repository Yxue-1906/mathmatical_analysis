      ...
      \begin{exercise}
          ...
          \begin{enumerate}
              \item 对每个$\varepsilon>0$,存在$N$,当$n>N$是,成立
                    ...
                    \end{solution}\item 对所有$n,p\in \mathbb{N}_+$,成立不等式$\lvert x_{n+p}-x_n\rvert\leqslant\frac{p}{n}$;
                    \begin{solution}
                        不一定是.显然基本数列满足这个条件,但例如$x_n=\sum_{i=1}^{n}\frac{1}{i}$,显然成立题设条件,甚至我们可以直接得到估计,但是我们知道$\lim_{n\to\infty}x_n=+\infty$,所以这不一定是基本数列.
                    \end{solution}\item 对所有$n,p\in \mathbb{N}_+$,成立不等式$\lvert x_{n+p}-x_n\rvert\leqslant\frac{p}{n^2}$;
                    \begin{solution}
                    ...
                    \end{solution}\item 对每个正整数$p$,成立$\lim_{n\to\infty}(x_n-x_{n+p})=0$.
                    \begin{solution}
